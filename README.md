# Hi, I'm Jay Talaviya 👋

### Python Developer | Backend Engineering | AI-Powered Applications

I’m a Python Developer focused on building production-grade backend systems and AI-powered applications. I work primarily with Python, Django, Django REST Framework, PostgreSQL, REST APIs, and modern AI/LLM technologies.

My experience includes building and maintaining production education platforms, AI-powered autograding systems, conversational and voice-based tutoring experiences, academic analytics, payment and subscription workflows, and end-to-end product features.

---

## 🚀 What I Build

- 🐍 Production backend services and REST APIs using Python and Django
- 🤖 AI-powered applications and LLM integrations
- 🎓 AI-enabled education and learning platforms
- 🧠 Automated assessment and multi-layer AI autograding systems
- 🎙️ Conversational and voice-based AI experiences
- 📊 Academic reporting, analytics, and performance insights
- 💳 Payment and subscription workflows
- 🔗 Third-party API integrations and backend services

---

## 🛠️ Tech Stack

### Backend

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Django](https://img.shields.io/badge/Django-092E20?style=flat&logo=django&logoColor=white)
![Django REST Framework](https://img.shields.io/badge/Django%20REST%20Framework-A30000?style=flat)
![REST API](https://img.shields.io/badge/REST%20APIs-02569B?style=flat)

### Databases

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)

### AI & LLM

![AI](https://img.shields.io/badge/AI%2FML-412991?style=flat)
![LLM](https://img.shields.io/badge/LLM%20Applications-6C47FF?style=flat)
![Conversational AI](https://img.shields.io/badge/Conversational%20AI-0A7EA4?style=flat)

### Frontend

![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)

### Tools & Integrations

![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)

---

## 💡 Featured Work

### SmartSchool

An AI-enabled education platform designed around teacher and student workflows.

**Key areas:**

- Teacher and student onboarding and workflow management
- Homework creation, submissions, evaluation, and management
- Multi-layer AI autograding with question-level evaluation and contextual feedback
- Multilingual assessment capabilities
- Academic performance analytics
- One-time and recurring payment workflows

### AI Tutor

An AI-powered learning platform focused on interactive and conversational learning.

**Key areas:**

- AI-powered chatbot for student doubt resolution
- Voice-based conversational tutoring
- Interactive question-practice workflows
- Conversational guidance for conceptual understanding

---

## 🎯 Engineering Focus

I enjoy working on problems involving:

- Backend architecture and API development
- AI/LLM-powered product features
- Production application development
- Database-driven systems
- API and third-party service integrations
- Automation and intelligent workflows
- Building reliable features from requirements to production

---

## 🏆 Certifications & Achievements

- **Microsoft Certified: Azure AI Fundamentals (AI-900)**
- **NPTEL Elite Certification: Problem Solving through Programming in C — 95% Score | IIT Kharagpur**

---

## 📫 Connect With Me

- **Email:** [talaviyajay10@gmail.com](mailto:talaviyajay10@gmail.com)
- **LinkedIn:** [linkedin.com/in/jay-talaviya-ab5b0b1b6](https://www.linkedin.com/in/jay-talaviya-ab5b0b1b6/)
- **GitHub:** [github.com/TALAVIYAJAY](https://github.com/TALAVIYAJAY)

---

### 💬 Always interested in building useful products, solving challenging backend problems, and exploring practical applications of AI.
